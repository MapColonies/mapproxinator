...
When set to **'true'**, creates the config file and initializes it. retrieves mapproxy configuration and updated time files from the provider. acts as an init-container.
...
`CONFIG_PROVIDER` available values: **'FS'**, **'S3'** or **'DB'**. default to **'FS'**.

determined where the mapproxy.yaml file is stored. no default value

**if set to 'FS'** - changes will apply directly to yaml file that declared in `MAPPROXY_YAML_FILEPATH`.
...
`S3_BUCKET` S3 bucket name, no default value
